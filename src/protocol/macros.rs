...
mod str_tests {
    ...
    fn test_serialize_string() {
        assert_eq_str!("\"test\"", serde_json::to_string(&Test).unwrap());
    }
...
mod hex_tests {
    ...
    fn test_hex_to_string() {
        assert_eq_str!("0x0", &Hex(0).to_string());
        assert_eq_str!("0x2a", &Hex(42).to_string());
    }

    #[test]
    fn test_hex_serialize() {
        assert_eq_str!("\"0x0\"", serde_json::to_string(&Hex(0)).unwrap());
        assert_eq_str!("\"0x2a\"", serde_json::to_string(&Hex(42)).unwrap());
    }

    #[test]
    fn test_hex_from_string() {
        assert_eq_dbg!(Hex(0), "0".parse().unwrap());
        assert_eq_dbg!(Hex(42), "42".parse().unwrap());
        assert_eq_dbg!(Hex(42), "0x2a".parse().unwrap());
        assert_eq_dbg!(Hex(42), "0X2A".parse().unwrap());
    }

    #[test]
    fn test_hex_deserialize() {
        assert_eq_dbg!(Hex(0), serde_json::from_str("\"0\"").unwrap());
        assert_eq_dbg!(Hex(42), serde_json::from_str("\"42\"").unwrap());
        assert_eq_dbg!(Hex(42), serde_json::from_str("\"0x2a\"").unwrap());
        assert_eq_dbg!(Hex(42), serde_json::from_str("\"0X2A\"").unwrap());
    }
